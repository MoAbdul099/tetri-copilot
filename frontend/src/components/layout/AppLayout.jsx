...
const NAV_CONFIG = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/compliance/intelligence', label: 'Intelligence',   icon: Brain },
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/ai/actions',        label: 'Action Center',  icon: Zap },
      { to: '/ai/approvals',      label: 'Approvals',      icon: CheckCircle },
      { to: '/ai/governance',     label: 'Governance',     icon: ShieldAlert },
      { to: '/ai/usage',          label: 'Usage',          icon: Activity },
      { to: '/ai/costs',          label: 'Costs',          icon: DollarSign },
      { to: '/ai/health',         label: 'Health',         icon: Gauge },
      ...
      { to: '/ai/diagnostics',    label: 'Diagnostics',    icon: AlertTriangle },
    ],
  },
  {
    type: 'group',
  ...
  return (
    <div className="flex h-screen bg-tetri-bg overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Desktop topbar */}
        <header className="hidden md:flex bg-tetri-surface border-b border-tetri-border h-14 items-center px-6 gap-4 flex-shrink-0">
          {/* Workspace switcher */}
          <div className="relative">
            <button
              onClick={() => hasMultipleWs && setWsSwitcherOpen((v) => !v)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${hasMultipleWs ? 'hover:bg-tetri-bg cursor-pointer' : 'cursor-default'}`}
            >
              <Building2 className="w-4 h-4 text-tetri-neutral flex-shrink-0" />
              <span className="font-semibold text-tetri-text truncate max-w-[200px]">{workspace?.name || '—'}</span>
              {hasMultipleWs && <ChevronDown className="w-3.5 h-3.5 text-tetri-muted flex-shrink-0" />}
            </button>
            {wsSwitcherOpen && hasMultipleWs && (
              <div className="absolute top-full left-0 mt-1 w-64 bg-tetri-surface border border-tetri-border rounded-xl shadow-lg py-1 z-50 max-h-60 overflow-y-auto">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-tetri-muted hover:bg-tetri-bg hover:text-tetri-text transition-colors"
              >
                ...
                <div className="text-left hidden lg:block">
                  <p className="text-xs font-semibold text-tetri-text leading-tight truncate max-w-[140px]">
                    {user?.fullName || user?.email}
                  </p>
                  <p className="text-xs text-tetri-neutral leading-tight truncate max-w-[140px]">{user?.email}</p>
                </div>
                <ChevronDown className="w-3.5 h-3.5 flex-shrink-0" />
              </button>

              {userMenuOpen && (
                <div className="absolute top-full right-0 mt-1 w-56 bg-tetri-surface border border-tetri-border rounded-xl shadow-lg py-1 z-50">
                  <div className="px-4 py-2.5 border-b border-tetri-border">
                    <p className="text-xs font-semibold text-tetri-text truncate">{user?.fullName || user?.email}</p>
        ...
        <header className="md:hidden bg-tetri-surface border-b border-tetri-border h-14 flex items-center px-4 gap-3 flex-shrink-0">
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <button
              onClick={() => setUserMenuOpen((v) => !v)}
              className="flex items-center p-1.5 rounded-lg text-tetri-muted hover:bg-tetri-bg transition-colors"
            >
            ...
            {userMenuOpen && (
              <div className="absolute top-14 right-4 w-56 bg-tetri-surface border border-tetri-border rounded-xl shadow-lg py-1 z-50">
                <div className="px-4 py-2.5 border-b border-tetri-border">
                  <p className="text-xs font-semibold text-tetri-text truncate">{user?.fullName || user?.email}</p>
                  <p className="text-xs text-tetri-neutral truncate">{user?.email}</p>
                </div>
