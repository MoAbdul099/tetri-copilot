...
const listInstances   = async (req, res, next) => { try { const d = await svc.listInstances(req.workspaceId, req.query); success(res, d); } catch (e) { next(e); } };
...
const resolveInstance = async (req, res, next) => { try { const d = await svc.resolveInstance(req.workspaceId, req.params.id); success(res, d); } catch (e) { next(e); } };
